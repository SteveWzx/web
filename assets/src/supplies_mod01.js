define(['jquery','page','datejs'],function($){
    ...
    $(function(){
        ...
        var getPage = function(index) {
            $.get('/v1/supplies?&limit=10&offset='+(10*index)+'',function(data){
                ...
                if(data  && log_length !== 0){
                    for(var i = 0; i < log_length; i++){
                        html += '<ul class="zc_fp">';
                        html += '<li class="zc_fpli zc_fpli01"><span class="zc_fpsp">'+data.Result[i].Sn+'</span><span  class="zc_fpsp">'+data.Result[i].Name+'</span><span  class="zc_fpsp">'+data.Result[i].Type+'</span></li>';
                        html += '<li class="zc_fpli zc_fpli05"><span class="zc_fpsp">'+data.Result[i].Sum+'</span><span class="zc_fpsp">'+data.Result[i].StoreSum+'</span><span class="zc_fpsp">'+data.Result[i].Color+'</span><span class="zc_fpsp">'+data.Result[i].Length+'</span></li>';
                        ...
                        html += '<li class="zc_fpli zc_fpli02"><span class="zc_fpsp">'+data.Result[i].Reserved+'</span><span class="zc_fpsp">'+data.Result[i].Notes+'</span></li>';
                        html += '</ul>';
                    }
                    $pannes.html(html);
                }
        ...
        $tnav.on('click',function(){
            ...
            $('.zc_fp>li').hide();
            $('.zc_fp>li:nth-child(4n+'+(ind+1)+')').show();
        });
