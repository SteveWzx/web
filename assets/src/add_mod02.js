    ...
    $(function(){
        ...
        $('.s_sub').on('click',function(){
            var $num = $.trim($('.s_num').val()),
                ...
                $cpu = parseInt($('.s_cpu').val()),
                $core = parseInt($('.s_core').val()),
                $memory = parseInt($('.s_memory').val()),
                $disk = parseInt($('.s_disk').val()),
            ...
            if($num === '' || $sn === '' || $name === '' || $brand === '' || $model === '' || isNaN($cpu) || isNaN($core) || isNaN($memory) || isNaN($disk) || $raid === '' || $time1 === '' || $time2 === '' || $time3 === '' || $time4 === '' || $admin === '' || $user === '' || $note === '' || isNaN($mid) || isNaN($aid) || isNaN($rid) || isNaN($did) || isNaN($bid) || isNaN($sid) || isNaN($status)){
                alert('输入有误，输入字段不能为空！');
            }else{
                var json = {
                    'AssetNumber': $num,
